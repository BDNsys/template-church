...
class TechGroupPermissionTests(TestCase):
    def setUp(self):
        ...
        self.tech_group = ChurchGroup.objects.create(name='Tech Team', group_type='TECH')
        self.factory = APIRequestFactory()

    def test_tech_maker_permission(self):
        # Assign MAKER role
        GroupMembership.objects.create(user=self.user, group=self.tech_group, role='MAKER')
    ...
    def test_tech_auditor_permission(self):
        # Assign AUDITOR role
        GroupMembership.objects.create(user=self.user, group=self.tech_group, role='AUDITOR')
